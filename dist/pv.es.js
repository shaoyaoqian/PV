...
const _PV = class {
  static initializeStore(data) {
    _PV.store = D(data);
    _PV.registerScope("globalStore", _PV.store);
  }
  static registerScope(name, scope) {
    _PV.scopes[name] = scope;
  }
  static registerDirective(name, directive) {
    _PV.directives.push([name, directive]);
  }
  static mount(mountPoint) {
    const app = Qe(_PV.scopes);
    _PV.directives.forEach((e) => app.directive(...e));
    app.mount(mountPoint);
    return app;
  }
};
let PV = _PV;
__publicField(PV, "store", D({}));
__publicField(PV, "scopes", {
